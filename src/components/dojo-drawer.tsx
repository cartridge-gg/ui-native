...
interface DojoDrawerProps {
	isOpen: boolean;
	onClose: () => void;
	onGameSelect?: (game: DojoGame) => void;
}

export function DojoDrawer({ isOpen, onClose, onGameSelect }: DojoDrawerProps) {
	...
	const loadGames = async () => {
		...
		try {
			...
			console.error("Failed to load Dojo games:", error);
		} finally {
			setLoading(false);
		}
	};
	...
	return (
		<View className="absolute inset-0 z-50">
			...
			<View
				className={cn(
					"absolute right-0 top-0 h-full w-80 bg-background-200 border-l border-background-300",
			...
			>
				...
				<View className="p-4 border-b border-background-300">
					<View className="flex-row items-center bg-background-300 rounded-lg px-3 py-2">
						<MagnifyingGlassIcon
							size="sm"
							className="text-foreground-400 mr-2"
						/>
