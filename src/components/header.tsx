...
import { HamburgerIcon, MagnifyingGlassIcon, Text } from "#components";
...
export function Header({
  ...
  return (
    <View
      className={cn("w-full", headerVariants[variant], className)}
      style={{
...
export function TabHeader({ title, subtitle, ...props }: HeaderProps) {
  return (
    <Header title={title} subtitle={subtitle} variant="elevated" {...props} />
  );
}
...
export function ArcadeHeader({
  ...
  return (
    <View
      className={cn("w-full bg-background-200", className)}
      style={{
        paddingTop: insets.top > 0 ? insets.top : 16,
      }}
    >
...
export function createBackHeader(
  ...
  return (
    <ScreenHeader title={title} onBack={onBack} rightAction={rightAction} />
  );
}
