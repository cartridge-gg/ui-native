...
export const Default: Story = {
  render: () => (
    <ProgressBar value={65} showLabel label="Loading..." />
  ),
};
  ...
  render: () => (
    <View style={{ gap: 24 }}>
      ...
      <View style={{ gap: 8 }}>
        ...
        <ProgressBar value={100} variant="success" showLabel label="Completed" />
      </View>
      
      <View style={{ gap: 8 }}>
        <Text variant="label">Warning</Text>
        <ProgressBar value={45} variant="warning" showLabel label="Warning State" />
      </View>
    ...
    return (
      <View style={{ gap: 24 }}>
        ...
        <View style={{ gap: 16 }}>
          ...
          <ProgressBar value={linearProgress} showLabel label="Linear Progress" />
          <View style={{ flexDirection: 'row', gap: 12 }}>
            <Button 
              title="-10" 
              onPress={() => setLinearProgress(Math.max(0, linearProgress - 10))}
              variant="secondary"
            />
            <Button 
              title="+10" 
              onPress={() => setLinearProgress(Math.min(100, linearProgress + 10))}
              variant="secondary"
            />
            <Button 
              title="Reset" 
        ...
        <View style={{ gap: 16, alignItems: 'center' }}>
          ...
          <View style={{ flexDirection: 'row', gap: 12 }}>
            <Button 
              title="-10" 
              onPress={() => setCircularProgress(Math.max(0, circularProgress - 10))}
              variant="secondary"
            />
            <Button 
              title="+10" 
              onPress={() => setCircularProgress(Math.min(100, circularProgress + 10))}
              variant="secondary"
            />
            <Button 
              title="Reset" 
        ...
        {skills.map((skill, index) => (
          <View key={index} style={{ gap: 4 }}>
            <ProgressBar 
              ...
              showLabel 
              label={skill.name}
              variant={skill.level >= 90 ? 'success' : skill.level >= 70 ? 'default' : 'warning'}
            />
          </View>
        ))}
      </View>
...
export const OnboardingFlow: Story = {
  render: () => {
    ...
    const steps = ['Welcome', 'Account Setup', 'Profile Info', 'Preferences', 'Complete'];
