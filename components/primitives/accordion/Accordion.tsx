...
import { View, Pressable, StyleSheet, ViewStyle, LayoutAnimation, Platform } from 'react-native';
...
export const AccordionItem: React.FC<AccordionItemProps> = ({
  ...
  const { type, value: accordionValue, onValueChange, collapsible, disabled: accordionDisabled } = context;
  const isDisabled = accordionDisabled || itemDisabled;
  
  const isOpen = type === 'single' 
    ? accordionValue === value
    : Array.isArray(accordionValue) && (accordionValue as string[]).indexOf(value) !== -1;
  ...
  return (
    <AccordionItemContext.Provider value={{ value, isOpen, onToggle: handleToggle, disabled: isDisabled }}>
...
export const AccordionTrigger: React.FC<AccordionTriggerProps> = ({
  ...
  return (
    ...
    >
      <View style={triggerStyles.content}>
        {children}
      </View>
      {!hideIcon && (
        <Text style={triggerStyles.icon}>▶</Text>
      )}
    </Pressable>
  );
};
