...
function findMatchingStories(pattern) {
	...
	return storyFiles.filter((file) => {
		...
		return (
			...
			regex.test(path.basename(file, ".stories.tsx"))
		);
	});
}
...
function updateSnapshots(storyFiles = []) {
	...
	try {
		const command = "npm run test:storybook:update";
		...
		if (storyPattern && storyFiles.length > 0) {
			...
			console.log(
				"⚠️  Note: Running all tests (test-storybook has limited filtering)",
			);
			console.log(
				"   The tool will update all snapshots, but you can verify only your target stories changed.",
			);
			console.log("");
		}
...
if (storyPattern && matchingStories.length === 0) {
	...
	console.log(
		'   • Use partial names (e.g., "Button" instead of "ButtonStories")',
	);
