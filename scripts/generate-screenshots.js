...
function analyzeScreenshots() {
	...
	const screenshots = files.filter(
		(f) => f.endsWith(".png") && !f.includes("-diff"),
	);
...
function generateReport() {
	...
	try {
		...
		console.warn(
			"⚠️  Could not generate visual comparison report:",
			error.message,
		);
	}
}
...
if (require.main === module) {
	...
	switch (command) {
		...
		case "-h":
			...
			console.log(
				"  --no-report       Skip generating visual comparison report",
			);
			...
			console.log(
				"  node scripts/generate-screenshots.js --no-cleanup --no-report",
			);
			break;
		default:
			main();
