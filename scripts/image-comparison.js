...
function generateVisualDiff(uiImage, nativeImage, outputPath) {
  try {
    ...
    try {
      ...
      try {
        const animatedGif = path.join(COMPARISON_OUTPUT_DIR, 'achievement-card-animated.gif');
...
function compareAchievementCard() {
  ...
  const outputImage = path.join(COMPARISON_OUTPUT_DIR, 'achievement-card-comparison.png');
  ...
  if (hasImageMagick) {
    ...
    const analysisResult = generateVisualDiff(uiImage, nativeImage, outputImage);
